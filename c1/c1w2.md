...
$$X$$表示一个$$n_x_m$$的训练样本矩阵,在python里就是`X.shape=(n_x,m)` $$Y$$表示一个$$1_m$$的向量,在python里是`Y.shape=(1,m)`
...
![gradient_descent_lr_m_examples](../.gitbook/assets/vectorization-lr.png) 如上图，将$$n_x$$维的dw变为一个np.array即可干掉内层的for loop。
...
* Reshape the datasets such that each example is now a vector of size (num_px  _num_px_  3, 1)
