...
- [1. setting up your machine learning application](#1-setting-up-your-machine-learning-application)
  - [1.1. train/dev/test sets](#11-traindevtest-sets)
  - [1.2. bias/variance](#12-biasvariance)
  - [1.3. basic recipe for machine learning](#13-basic-recipe-for-machine-learning)
- [2. regularizing your neural network](#2-regularizing-your-neural-network)
  - [2.1. regularization](#21-regularization)
  - [2.2. why regularization reduces overfitting?](#22-why-regularization-reduces-overfitting)
  - [2.3. dropout regularization](#23-dropout-regularization)
  - [2.4. understanding dropout](#24-understanding-dropout)
  - [2.5. other regularization methods](#25-other-regularization-methods)
    - [2.5.1. data augmentation](#251-data-augmentation)
    - [2.5.2. early stopping](#252-early-stopping)
- [3. setting up your optimization problem](#3-setting-up-your-optimization-problem)
  - [3.1. normalizing inputs](#31-normalizing-inputs)
  - [3.2. vanishing/exploding gradients](#32-vanishingexploding-gradients)
  - [3.3. weight initialization for deep networks](#33-weight-initialization-for-deep-networks)
  - [3.4. numerical approximation of gradients](#34-numerical-approximation-of-gradients)
  - [3.5. gradient checking](#35-gradient-checking)
  - [3.6. gradient checking implementation notes](#36-gradient-checking-implementation-notes)
...
![train-dev-test-ratio.png](../c2/imgs/train-dev-test-ratio.png)

另外，训练集和验证集的分布要保持一致。测试集是为了no-bias地验证模型效果的，有些时候，可以甚至不要测试集，只要验证集就可以了。

![distribution-between-train-and-dev.png](../c2/imgs/distribution-between-train-and-dev.png)

## 1.2. bias/variance

![bias-variance-under-overfitting.png](../c2/imgs/bias-variance-under-overfitting.png)
...
![bias-variance-high-low.png](../c2/imgs/bias-variance-high-low.png)

对应模型的最优复杂度如下：

![bias-variance-high-low-complexity.png](../c2/imgs/bias-variance-high-low-complexity.png)

当optimal error(bayes error)约等于0%时（错误率是0，全部才能识别出来），如下图所示。当optimal error是15%时，第二个分类器就是low bias了。

![bias-variance-cat-classification.png](../c2/imgs/bias-variance-cat-classification.png)

最差情况就是两个都high的，如下紫色曲线（线性部分是high bias，因为有很多没分对的情况；中间两个点是high variance，overfitting了）:

![bias-variance-worst.png](../c2/imgs/bias-variance-worst.png)
...
![basic-recipe-for-machine-learning.png](../c2/imgs/basic-recipe-for-machine-learning.png)
...
![regression-lr.png](../c2/imgs/regression-lr.png)
...
![neural-network-lr.png](../c2/imgs/neural-network-lr.png)

## 2.2. why regularization reduces overfitting?

直观地看，当$\lambda$很大时，最小化loss，会使得w趋向0，这样，相当于很多神经元无形地被干掉了，模型变简单了。

![how-does-regularization-reduces-overfitting.png](../c2/imgs/how-does-regularization-reduces-overfitting.png)
...
![how-does-regularization-reduces-overfitting-2.png](../c2/imgs/how-does-regularization-reduces-overfitting-2.png)

## 2.3. dropout regularization

简单理解，dropout就是对每一层设置一个dropout rate，在训练时，针对每一条训练样本，以这个比例把某些神经元及与其连接的权重直接去掉。

![dropout-regularization-introduction.png](../c2/imgs/dropout-regularization-introduction.png)
...
![dropout-regularization-implementation.png](../c2/imgs/dropout-regularization-implementation.png)

test阶段，**不要使用dropout**，因为我们并不想让预测的结果是random的，或者是有噪音的。如果用了inverted dropout，**在test的时候，就不需要做这个/=keep_prob的操作了。**

![dropout-regularization-test-implementation.png](../c2/imgs/dropout-regularization-test-implementation.png)
...
![dropout-regularization-understanding-dropout.png](../c2/imgs/dropout-regularization-understanding-dropout.png)
...
![other-regularization-data-augmentation.png](../c2/imgs/other-regularization-data-augmentation.png)
...
![other-regularization-early-stopping.png](../c2/imgs/other-regularization-early-stopping.png)
...
![normalizing-training-sets.png](../c2/imgs/normalizing-training-sets.png)
...
![why-normalize-inputs.png](../c2/imgs/why-normalize-inputs.png)
...
![vanishing-exploding-gradients.png](../c2/imgs/vanishing-exploding-gradients.png)
...
如果用的是ReLU，那么，把上面的1改成2【**He initialization**,He et al., 2015】：
$$var(w_i)=\frac{2}{n}$$
...
![weight-initialization.png](../c2/imgs/weight-initialization.png)

## 3.4. numerical approximation of gradients

有时写完代码后，不确定梯度算得有没有问题，可以通过逼近的方法来检查梯度。

![numerical-approximation-of-gradients.png](../c2/imgs/numerical-approximation-of-gradients.png)
...
![gradient-check-for-a-neural-network.png](../c2/imgs/gradient-check-for-a-neural-network.png)

gradient check的步骤如下（$\epsilon$常取$10^{-7}$）,当下式约等于$10^{-7}$时，应该没问题，如果大于$10^{-3}$，很可能有问题。

$$ \frac{||d\theta_{approx}-d\theta||_2}{||d\theta_{approx}||_2+||d\theta||_2} $$

![gradient-check.png](../c2/imgs/gradient-check.png)
...
![gradient-check-implementation-notes.png](../c2/imgs/gradient-check-implementation-notes.png)
