...
- [1. logistic regression as a neural network](#1-logistic-regression-as-a-neural-network)
  - [1.1. binary classification](#11-binary-classification)
  - [1.2. logistic regression](#12-logistic-regression)
  - [1.3. logistic regression cost function](#13-logistic-regression-cost-function)
  - [1.4. gradient descent](#14-gradient-descent)
  - [1.5. derivatives](#15-derivatives)
  - [1.6. more derivative examples](#16-more-derivative-examples)
  - [1.7. computation graph](#17-computation-graph)
  - [1.8. derivatives with a computation graph](#18-derivatives-with-a-computation-graph)
  - [1.9. logistic regression gradient descent](#19-logistic-regression-gradient-descent)
  - [1.10. gradient descent on m examples](#110-gradient-descent-on-m-examples)
- [2. python & vectorization](#2-python--vectorization)
  - [2.1. vectorization](#21-vectorization)
  - [2.2. more examples of vectorization](#22-more-examples-of-vectorization)
  - [2.3. vectorizing logistic regression](#23-vectorizing-logistic-regression)
  - [2.4. vectorizing logistic regression's gradient output](#24-vectorizing-logistic-regressions-gradient-output)
  - [2.5. broadcasting in python](#25-broadcasting-in-python)
  - [2.6. a note on python/numpy vectors](#26-a-note-on-pythonnumpy-vectors)
  - [2.7. quick tour of jupyter/ipython notebooks](#27-quick-tour-of-jupyteripython-notebooks)
  - [2.8. explanation of logistic regression cost function](#28-explanation-of-logistic-regression-cost-function)
  - [2.9. programming assignments](#29-programming-assignments)
...
KKKL(\hat {y}, y)=1/2*(\hat {y} - y)^2KKK
+ logistic regression的loss function: 
KKKL(\hat {y}, y)=-(ylog\hat {y}+(1-y)log(1-\hat {y}))$$
...
KKKJ(w,b)=1/m\sum ^m_{i=1}L(\hat {y}^{(i)}, y^{(i)})=-1/m\sum^m_{i=1}[y^{(i)}log\hat {y}^{(i)}+(1-y^{(i)})log(1-\hat {y}^{(i)})]$$
...
![computation graph](../c1/imgs/computation%20graph.png)

## 1.8. derivatives with a computation graph
KKKJ=3v,v=a+u,u=bcKKK
$$\frac {dJ}{dv}=3, \frac{dv}{da}=1KKK
KKKso, \frac {dJ}{da}=\frac {dJ}{dv}\frac {dv}{da}=3*1=3KKK
KKKif\ b=2,then\ \frac{dJ}{dc}=\frac{dJ}{dv}\frac{dv}{du}\frac{du}{dc}=3*1*b=3*1*2=6KKK
**写代码时，将$$\frac{dFinalOutputVar}{dvar}$$记为$$dvar$$(最后输出对这个变量的偏导)**

![derivatives in computation graph](../c1/imgs/computation%20graph-derivatives.png)

## 1.9. logistic regression gradient descent

KKKz=w^Tx+bKKK
$$\hat {y}=a=\sigma (z)$$
KKKL(a,y)=-(ylog(a)+(1-y)log(1-a))$$

![derivatives in computation graph in lr](../c1/imgs/computation%20graph-derivatives-lr.png)

## 1.10. gradient descent on m examples

首先，根据J的公式，可以知道dJ/dw1其实就是对每个样本的dw1求和，然后/m。

![gradient_descent_lr_m_examples_djdw](../c1/imgs/gradient_descent_lr_m_examples_djdw.png)

每一次迭代，遍历m个样本，算出J/dw1/dw2/db，然后用这些梯度去更新一次w1/w2/b。

![gradient_descent_lr_m_examples](../c1/imgs/gradient_descent_lr_m_examples.png)
...
![gradient_descent_lr_m_examples](../c1/imgs/vectorization-1.png)

对于两个100w维的向量进行点乘，vectorization(1.5ms) 比for loop(470ms+)快

## 2.2. more examples of vectorization

![gradient_descent_lr_m_examples](../c1/imgs/vectorization-2.png)

![gradient_descent_lr_m_examples](../c1/imgs/vectorization-3.png)

![gradient_descent_lr_m_examples](../c1/imgs/vectorization-lr.png)
...
![gradient_descent_lr_m_examples](../c1/imgs/vectorization-lr-2.png)

## 2.4. vectorizing logistic regression's gradient output

![gradient_descent_lr_m_examples](../c1/imgs/vectorization-lr-3.png)

![gradient_descent_lr_m_examples](../c1/imgs/vectorization-lr-4.png)
...
![broadcasting](../c1/imgs/broadcasting.png)
