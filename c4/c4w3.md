...
然后对原图，拿滑动窗口慢慢滑动，每一个小区域过一遍convnet，看看是否是car。这算做一次slide。
...
+ 对于中间那行，左右两个有car的中心点，所以$p_c=1$，而中间那个grid cell，虽然有一些边界，但没有中心点，所以仍然$p_c=0$。
...
如果有3个类别，$c_1,c_2,c_3$，则需要对每个类别独立地进行一次如上操作。
